...
/* same as m_textregion, only use rows and columns instread of pixels */

#define m_termregion(x,y,wide,high) \
	(fprintf(m_termout,"%c%d,%d,%d,%d,%c", \
	m_escchar,x,y,wide,high,E_TEXTREGION)_m_flsh)
...
extern int get_info(), get_size(), mparse(), get_font(), is_active(), menu_load();

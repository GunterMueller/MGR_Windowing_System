 ...
 */
/*	$Header: do_button.c,v 1.2 89/03/17 08:45:13 sau Exp $
	$Source: /m1/mgr.new/src/RCS/do_button.c,v $
*/
static char	RCSid_[] = "$Source: /m1/mgr.new/src/RCS/do_button.c,v $$Revision: 1.2 $";
           ...
           if (win == active || win == (WINDOW *) 0) {
              if (active && win == active) {
                 state = menu_define(font,active_menu,0,0,
                         PUTCOLOR(W(style)),
                         PUTCOLOR(W(background)));
                 which_menu = 1;
                 }
              else if (next_window >= MAXWIN) {
                 state = menu_define(font,full_menu,0,0,MENU_FG,MENU_BG);
                 which_menu = 2;
                 }
              else {
                 state = menu_define(font,main_menu,0,0,MENU_FG,MENU_BG);
                 which_menu = 3;
                 }
              if (active) {
                 cursor_off();
...
int quit()
   ...
   state = menu_define(font,quit_menu,0,0,MENU_BG,MENU_FG);
   state = menu_setup(state,screen,mousex,mousey,0);
...
_quit()
   ...
   if (really_quit++) {			/* we're in bad shape */
...
/* redraw the screen, restore contents of saved windows */

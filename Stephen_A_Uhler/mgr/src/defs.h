 ...
 */
/*	$Header: /home/sau/mgr/src/RCS/defs.h,v 1.1 91/02/11 09:45:59 sau Exp Locker: sau $
	$Source: /home/sau/mgr/src/RCS/defs.h,v $
*/
static char	h_defs_[] = "$Source: /home/sau/mgr/src/RCS/defs.h,v $$Revision: 1.1 $";
...
#ifdef FASTMOUSE
#define MoUSE(a,b)     bit_blit(screen, a, b, 16,16, \
                               BIT_SRC^BIT_DST, m_rop,0,0)
#define MOUSE_ON(a,b)	(!mouse_on && (mouse_on=1, MoUSE(a,b)) )
#define MOUSE_OFF(a,b)	( mouse_on && (mouse_on=0, MoUSE(a,b)) )
#else
/* turn on the mouse only if it is off */
#define MOUSE_ON(a,b)     (!mouse_on && ( \
				  mouse_on=1, \
			          bit_blit(&mouse_save,0,0,16,16, \
					BIT_SRC,screen,a,b), \
				  bit_blit(screen,a,b,16,16, \
					BIT_SRC|BIT_DST,m_rop,0,0), \
				  bit_blit(screen,a,b,16,16, \
				      BIT_NOT(BIT_SRC)&BIT_DST,m_rop,0,16) \
				  ))
/* turn off the mouse only if it is on */
#define MOUSE_OFF(a,b)     (mouse_on && ( \
			          mouse_on=0, \
			          bit_blit(screen,a,b,16,16, \
					BIT_SRC,&mouse_save,0,0) \
			          ))
#endif
...
extern BITMAP mouse_save, mouse_not;

 ...
 */
/*	$Header: /home/sau/mgr/src/RCS/put_window.c,v 1.1 91/02/11 09:47:58 sau Exp Locker: sau $
	$Source: /home/sau/mgr/src/RCS/put_window.c,v $
*/
static char	RCSid_[] = "$Source: /home/sau/mgr/src/RCS/put_window.c,v $$Revision: 1.1 $";
...
#define PUT_CHAR(dest,x,y,font,op,c)   \
   ...
   BIT_SRC | (W(background)&~NOCOLOR),BIT_NULL,0,0), \
   bit_blit(dest,x,y-fsizehigh,fsizewide,fsizehigh, \
   (op & NOCOLOR)^BIT_DST | (op ^ W(background))&~NOCOLOR, \
   DO_CHAR(font,c),0,0)
   ...
   case W_ESCAPE:			/* process an escape code */
      ...
      switch(c&=0177) {
         ...
         case E_ADDLINE:  		/* add a new line */
               if (*W(esc)) {
                  ...
                  scroll(win,text,W(y)-fsizehigh,T_HIGH,-(fsizehigh),
                      W(background));
                  bit_blit(text,0,HIGH-fsizehigh,T_WIDE,
                      fsizehigh,W(background),NULL_DATA,0,0);
                  }
               done++;
               break;
         ...
         case E_FCOLOR:       /* set forground color */
               W(style) = (W(style)&NOCOLOR) | GETCOLOR(*W(esc));
               BORDER(win);
               break;

         case E_BCOLOR:       /* set background color */
               W(background) = (W(background)&NOCOLOR) | GETCOLOR(*W(esc));
               BORDER(win);
               break;
         ...
         case E_CLEAREOL:  		/* clear to end of line */
               bit_blit(text,W(x),W(y)-fsizehigh,T_WIDE-W(x),
                    fsizehigh,W(background),0,0,0);
               if (Do_clip())
         ...
         case E_CLEAREOS:  		/* clear to end of window */
               bit_blit(text,W(x),W(y)-fsizehigh,T_WIDE-W(x),
                    fsizehigh,W(background),0,0,0);
               bit_blit(text,0,W(y),T_WIDE,T_HIGH-W(y),W(background),0,0,0);
               if (Do_clip())
         ...
         case E_BLEEP:			/* highlight a section of the screen */
               if (cnt>2) {
                  ...
                  bit_blit(screen,p[0],p[1],p[2],p[3],BIT_NOT(BIT_DST),NULL_DATA,0,0);
                  fsleep();
                  bit_blit(screen,p[0],p[1],p[2],p[3],BIT_NOT(BIT_DST),NULL_DATA,0,0);
                  done++;
                  }
         ...
         case E_FONT:  			/* pick a new font */
               ...
               if (cnt > 0) {
                  ...
                  if (W(y) < fsizehigh) {
                     scroll(win,text,W(y)-fsizehigh,T_HIGH,
                            W(y)-fsizehigh,W(background));
                     W(y)=fsizehigh;
                     done++;
                     }
                  }
         ...
         case E_BITCRT:  		/* create/destroy a bitmap */
               switch(cnt) {
                  case 0:
                     ...
                     break;
                  case 2:		/* create a new bitmap */
                     if (W(esc)[0] && W(esc[0]<=MAXBITMAPS) &&
                                  W(bitmaps)[W(esc)[0]-1] == (BITMAP *) 0) {
                        W(bitmaps)[W(esc)[0]-1] = 
                              bit_alloc(Scalex(W(esc)[1]),Scalex(W(esc)[2]),
                                        NULL_DATA,DEPTH);
#ifdef DEBUG
         ...
         case E_BITLOAD:  		/* download a bitmap */
               if (cnt >=2) {
						int format = bmap_size(W(esc[0]),W(esc[1]),W(esc[cnt]));
                  W(esc)[TEXT_COUNT] = 0;
                  if (format==DEPTH)
                     W(bitmap) =bit_alloc(W(esc[0]),W(esc[1]),NULL_DATA,DEPTH);
                  else
                     W(bitmap) =bit_alloc(W(esc[0]),W(esc[1]),NULL_DATA,1);
                      ...
                      if (W(esc)[0] >=0 && W(esc)[1] >= W(esc)[0] && 
                         ...
                         if (W(y) > W(text.high))
                            W(y) = W(text.high);
							 ...
							 /* not SIGWINCH on scrolling regions (emacs botch) */
                      break;
		      ...
		      break;
                 ...
                 case M_WOB:		/* set white on black */
                      if (W(flags)&W_REVERSE) 
                         break;
#ifdef COLOR
                      W(style) = W(style)&NOCOLOR |
                            (~W(style))&~NOCOLOR;
                      W(background) = W(background)&NOCOLOR |
                            (~W(background))&~NOCOLOR;
                      CLEAR(window,W(background));
                      BORDER(win);
#else
                      W(background) = BIT_SET;
                      W(flags)|= W_REVERSE;
                      if (W(flags)&W_OVER)
                         W(style) = ROP_INVERT(W(style));
                      else
                         W(style)= W(flags)&W_STANDOUT ?
                                          BIT_SRC : BIT_NOT(BIT_SRC);
                      CLEAR(window,BIT_NOT(BIT_DST));
#endif
                      if (Do_clip())
                         Set_all();
                      break;
                 ...
                 case M_OVERSTRIKE:	/* turn on overstrike */
                      W(style) = W(style)&~NOCOLOR | W(op)&NOCOLOR;
                      W(flags) |= W_OVER;
                      break;
		 ...
		 case M_STANDOUT:
		      ...
		      break;
                 ...
                 case M_WOB:		/* set black-on-white */
                      ...
                      BORDER(win);
#else
                      W(background) = BIT_CLR;
                      W(flags)&= ~W_REVERSE;
                      if(W(flags)&W_OVER)
                         W(style) = ROP_INVERT(W(style));
                      else
                         W(style)= W(flags)&W_STANDOUT ?
                                BIT_NOT(BIT_SRC) : BIT_SRC;
                      CLEAR(window,BIT_NOT(BIT_DST));
#endif
                      if (Do_clip())
                         Set_all();
                 ...
                 case M_OVERSTRIKE:	/* turn off overstrike */
                      if ( !(W(flags)&W_STANDOUT) ^ !(W(flags)&W_REVERSE))
#ifdef COLOR
                         W(style) = BIT_SRC | ~NOCOLOR & W(background);
                      else
                         W(style) = BIT_SRC | ~NOCOLOR & W(style);
#else
                         W(style) = BIT_NOT(BIT_SRC);
                      else
                         W(style) = BIT_SRC;
#endif
                      W(flags) &= ~W_OVER;
...
*/
                    PUT_CHAR(text,W(x),W(y),W(font),W(style),c);
...
register WINDOW *win;
{
	...
	if (W(flags)&W_OVER) 
		...
		W(style) = W(flags)&W_REVERSE ? BIT_SRC : BIT_NOT(BIT_SRC);
#endif
	W(flags) |= W_STANDOUT;
}
...
register WINDOW *win;
{
	...
	if (W(flags)&W_OVER) 
		...
		W(style) = W(flags)&W_REVERSE ? BIT_NOT(BIT_SRC) : BIT_SRC;
#endif
	W(flags) &= ~ W_STANDOUT;
}

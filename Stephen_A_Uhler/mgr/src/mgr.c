 ...
 */
/*	$Header: /home/sau/mgr/src/RCS/mgr.c,v 1.2 91/02/07 14:11:45 sau Exp Locker: sau $
	$Source: /home/sau/mgr/src/RCS/mgr.c,v $
*/
static char	RCSid_[] = "$Source: /home/sau/mgr/src/RCS/mgr.c,v $$Revision: 1.2 $";
...
char **argv;
         ...
         case 1:	/* looking for flag */
              if (**argv == '-')
                 ...
                 case 'V': 		/* print version and options */
                           ...
                           printf("Compile flags: (%s) %s\n",
												version[7],version[3]);
                           printf("Graphics library: %s\n", version[8]);
   ...
   if ((mouse=open(mouse_dev,2)) <0) {
      fprintf(stderr,"can't find the mouse, or it is already in use\n");
		sleep(2);
      }
   ...
   while(1) {
      ...
      dprintf(l)(stderr,"select: mask=0x%x poll=0x%x 0x%x got\r\n",
                 mask,poll,reads);
		if (debug) alarm(0);		/* watchdog timer */
#endif
#ifdef MOVIE
		DOFLUSH();
#endif
      ...
      dprintf(l)(stderr,"0x%x\r\n",reads);
		if (debug) alarm(30);		/* watchdog timer */
#endif

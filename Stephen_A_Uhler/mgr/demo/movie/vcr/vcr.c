...
int time();				/* counter timer int handler */
char *get_names();
...
char **argv;
	...
	int tw, th;		/* total icon width, height */
	...
	get_icon(SMP,BUTTON_NAME,&tw,&th);
...
get_icon(n,name,tw,th)
int n;					/* icon number */
char *name;				/* icon name */
int *tw, *th;				/* icno size */
	{
	m_bitfile(n,name,tw,th);

	/* can't find icon, download my own */

	if (*tw==0) {
		int local_mode;		/* local mode for tty */
		int size;				/* size of icon */
		extern BITMAP button_icon;	/* button_data */
	
		/* try to enable 8 bit output */
      ...
      ioctl(fileno(m_termout),TIOCLSET,&local_mode);
		...
		size = button_icon.high * (((button_icon.wide+31)&~31)>>3);
      m_bitldto(button_icon.wide,button_icon.high,0,0,n,size);
      m_flush();
      write(fileno(m_termout),button_icon.data,size);

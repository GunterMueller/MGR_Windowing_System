...
#define WIDE	1152
#define HIGH	900
#define SCALE	(1<<16)				/* max unsigned short */
...
char **argv;
	...
	if (!quiet && (display=bit_open("/dev/bwtwo0"))==NULL) {
		fprintf(stderr,"Can't open the display\n");
		exit(1);
		}
	else if (quiet)
		display = bit_alloc(WIDE,HIGH,NULL,1);

	bzero(maps,sizeof(maps));		/* initialize the bitmaps */
	...
	while(1) {
		...
		/* got control input from vcr */

		if (do_pause(vcr_file,wait) > 0) {		/* got vcr input */
			if (!gets(line))
				break;
	...
	if (!quiet)
		bit_blit(screen,0,0,BIT_WIDE(screen),BIT_HIGH(screen),BIT_CLR,0,0,0);
	bit_destroy(screen);
	close_script(file);
	printf("Q\n");
	exit(0);
...
int
grind(screen,file,speed)
BITMAP *screen;	/* the destination (screen) */
FILE *file;		/* file to read commands from */
int speed;		/* how fast to process */
	{
	register int c;						/* command type */
	BITMAP *new;							/* newly created bitmap */
	register char *data;					/* pointer to bitmap data */

	if (file == NULL || speed <=0)
		return(0);
		...
		switch (c&TYPE_MASK) {
			...
			case T_DATA:					/* download some data */
			   ...
			   qprintf(stderr," downloading  %d\n", args[3]);
				if (args[3] != 0 && maps[args[0]]==screen) { /* the screen */
			...
			case T_TIME:					/* get a timestamp */
				...
				if (your_time < old_time) {
					...
					if (old_time - scale > SCALE-THRES) {	/* overflow */
...
/* get a 2-byte quantity (byte order independent) */

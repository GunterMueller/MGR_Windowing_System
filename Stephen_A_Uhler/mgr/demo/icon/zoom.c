 ...
 */
/*	$Header: zoom.c,v 4.4 88/07/11 13:10:39 sau Exp $
	$Source: /tmp/mgrsrc/demo/icon/RCS/zoom.c,v $
*/
static char	RCSid_[] = "$Source: /tmp/mgrsrc/demo/icon/RCS/zoom.c,v $$Revision: 4.4 $";

/* icon edittor -- version I (single window) */
...
#define TYPE	1		/* mgr bitmap type 0=old, 1=new */
...
char **argv;
   ...
   for(i=1;i<argc && i<= FILES-3;i++) {		/* save room for new names */
      ...
      files[i-1].action = str_save(argv[i],"\r");
      }
   file_count = i-1;
   ...
   m_setevent(BUTTON_1,"(%r)\r");
   m_setevent(BUTTON_2,"[%p]\r");
   m_setevent(BUTTON_2U,"$\r");
   m_setevent(RESHAPED,"S\r");
...
int count;			/* # of menu items */
   ...
   *input = '\0';
   if (win) {
      m_selectwin(win);
      m_setevent(DEACTIVATED,"\r");
      m_setevent(DESTROY,"\r");
      m_setevent(RESHAPE,"\r");
      m_setevent(BUTTON_1,"%n\r");
      m_setevent(ACCEPT,"%m\r");
      if (count > 0) {
...
BITMAP *map;
   ...
   if (fp == NULL  ||  !bitmapwrite(fp,map,TYPE)) {
...
int x,y;
	{
	return(bit_point(map,x,y,BIT_DST));
	}

...
#ifndef VERSION
#define VERSION "PGS 1.0 (c) 1992 Bellcore, by S A Uhler:"
#endif
#ifndef GS
#define GS "gs -r%d -sDEVICE=mgr -q -sOUTPUTFILE=%s %s"
#endif
#define TMP_FILE	"%s/pgs_%d.%%d"
#define TITLE		"\r%s: page %d of %d %s\033c"
...
struct menu_entry page_menu[] = {
	...
	"11", "P 11", "12", "P 12", "13", "P 13", "14", "P 14",
	"15", "P 15", "16", "P 16", "17", "P 17", "18", "P 18",
	"19", "P 19", "20", "P 20",
	};
...
char **argv;
   ...
   for(i=1;i<argc;i++) {
	...
	sprintf(temp_file,TMP_FILE,temp,getpid());
	...
	fprintf(m_termout,"\r%s previewing file %s ",VERSION,name);

	while(1) {
		reads = mask;
		m_flush();
		if (select(32,&reads,0,0,0L) < 0) {
			dprintf(stderr,"select error\n");
			continue;
			}
		dprintf(stderr,"reads returned 0x%x\n",reads);

		if (reads&(1<<fd)) {						/* from MGR */
			read(fd,&c,1);
			...
			case 'Q': case 'q': case '\004':
				read(fd,&c,1);		/* swallow the button-up */
				cleanup("Bye now!");
				break;
			case 'g': /* goto page (from keyboard) */
				fprintf(m_termout,"\rpage: \033c",c);
				m_setcursor(0);
				m_setnoraw();
				m_setecho();
...
int _fd;				/* fd to read from */
	...
	dprintf(stderr,"Get.."); dflush(stderr);
	m_flush();
	while((n=read(_fd,s,1))==1) {		/* single char reads, YUK ! */
		dprintf(stderr,"%c",*s); dflush(stderr);
		if (index(term,*s)) {
			if (i==0) {
...
char *s;		/* extra stuff */
	{
	fprintf(m_termout,TITLE,name,num,max,s?s:"");
	m_flush();
	}

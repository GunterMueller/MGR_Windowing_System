 ...
 */
/*	$Header: /home/sau/mgr/demo/misc/stat2.c,v 1.1 91/02/04 10:24:18 sau Exp Locker: sau $
	$Source: /home/sau/mgr/demo/misc/stat2.c,v $
*/
static char	RCSid_[] = "$Source: /home/sau/mgr/demo/misc/stat2.c,v $$Revision: 1.1 $";
...
struct data {
   ...
   int start;		/* starting offset in input line */
   int width;		/* field width */
   int max;	   	/* maximum value */
   };

struct data data[] = {
   "r",		"jobs in run q",	 0, 2,	5,
   "b",		"jobs blocked",		 2, 2,	5,
   "w",		"jobs waiting",		 3, 2,	5,
   "fre",	"free memory",		12, 6,	20000,
   "fr",	"freed pages",		33, 4,	20,
   "d1",	"disk 1 accesses",	45, 3,	20,
   "d2",	"disk 2 accesses",	48, 3,	20,
   "d3",	"disk 3 accesses",	51, 3,	20,
   "d4",	"disk 4 accesses",	54, 3,	20,
   "in",	"interrupts",		57, 4,	1000,
   "sy",	"system calls",		61, 4,	150,
   "cs",	"context switches",	65, 4,	75,
   "us",	"% user time",		69, 3,	100,
   "kn",	"% system time",	72, 3,	100,
   "id",	"% idle time",		75, 3,	100,
   "",		"",			-1,-1,	-1
   };
...
char **argv;
   ...
   while (1) {
      ...
      while (fgets(line,sizeof(line),file) != NULL) {
			dprintf(stderr,"got: %s",line);
         if (strncmp(line," procs",6)==0) {
         ...
         for(i=0;i<count;i++) {
            sscanf(line+data[field[i]].start,
                  formats[data[field[i]].width],&value);
            current[i] = value * (y2-3)/data[field[i]].max;
				warn = (current[i] > y2-3);

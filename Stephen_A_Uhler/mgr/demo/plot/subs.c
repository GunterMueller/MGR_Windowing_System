 ...
 */
/*	$Header: subs.c,v 4.1 88/06/21 14:03:15 bianchi Exp $
	$Source: /tmp/mgrsrc/demo/plot/RCS/subs.c,v $
*/
static char	RCSid_[] = "$Source: /tmp/mgrsrc/demo/plot/RCS/subs.c,v $$Revision: 4.1 $";
...
static int debug;			/* iff TRUE debug -> stderr */
...
openpl()
   ...
   dprintf(stderr,"OPEN\n");
   m_setup( debug ? M_DEBUG|M_FLUSH : M_DEBUG);
   m_push(P_FLAGS);
   m_func(B_SET);
   if (win_id) {
      m_selectwin(win_id);
